...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['create'])) {
  if (!csrf_verify($_POST['csrf_token'] ?? '')) {
    header("Location: manage_about.php?error=csrf");
    exit;
  }
    ...
    <?php while ($about = $result->fetch_assoc()): ?>
      <div class="card">
        ...
        <form method="POST" class="edit-form">
          ...
          <a href="manage_about.php?delete=<?= $about['id']; ?>&csrf_token=<?= urlencode(csrf_token()) ?>" class="btn logout" onclick="return confirm('Are you sure you want to delete this entry?')">Delete</a>
        </form>
      </div>
    <?php endwhile; ?>
  </div>
