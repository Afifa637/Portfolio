...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_skill'])) {
    if (!csrf_verify($_POST['csrf_token'] ?? '')) {
        header("Location: manage_skills.php?error=csrf");
        exit;
    }
            ...
            <?php while ($row = $result->fetch_assoc()): ?>
                <tr>
                    ...
                    <td>
                        ...
                        <a href="?delete=<?= $row['id'] ?>&csrf_token=<?= urlencode(csrf_token()) ?>" class="btn delete-btn" onclick="return confirm('Delete this skill?');">Delete</a>
                    </td>
                </tr>
            <?php endwhile; ?>
        </table>
