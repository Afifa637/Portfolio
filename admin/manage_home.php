...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_home'])) {
    if (!csrf_verify($_POST['csrf_token'] ?? '')) {
        header("Location: manage_home.php?error=csrf");
        exit;
    }
        ...
        <ul class="list">
            <?php while ($role = $roles->fetch_assoc()): ?>
                <li><?= htmlspecialchars($role['role']) ?>
                    <a href="?delete_role=<?= $role['id'] ?>&csrf_token=<?= urlencode(csrf_token()) ?>" class="btn logout">Delete</a>
                </li>
            <?php endwhile; ?>
        </ul>
        ...
        <ul class="list">
            <?php while ($social = $socials->fetch_assoc()): ?>
                <li><?= htmlspecialchars($social['platform']) ?> - <?= htmlspecialchars($social['url']) ?>
                    <a href="?delete_social=<?= $social['id'] ?>&csrf_token=<?= urlencode(csrf_token()) ?>" class="btn logout">Delete</a>
                </li>
            <?php endwhile; ?>
        </ul>
