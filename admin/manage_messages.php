...
if (isset($_GET['delete'])) {
    if (!csrf_verify($_GET['csrf_token'] ?? '')) {
        header("Location: manage_messages.php?error=csrf");
        exit;
    }
    $id = (int)$_GET['delete'];
                ...
                <tbody>
                    <?php while ($row = $result->fetch_assoc()): ?>
                        <tr>
                            ...
                            <td>
                                <a href="?delete=<?= $row['id'] ?>&csrf_token=<?= urlencode(csrf_token()) ?>" class="btn delete-btn" onclick="return confirm('Delete this message?');">Delete</a>
                            </td>
                        </tr>
                    <?php endwhile; ?>
                </tbody>
