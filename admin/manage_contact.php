...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_contact'])) {
  if (!csrf_verify($_POST['csrf_token'] ?? '')) {
    header("Location: manage_contact.php?error=csrf");
    exit;
  }
        ...
        <?php while ($row = $result->fetch_assoc()): ?>
          <tr>
            ...
            <td>
              ...
              <a href="?delete=<?= $row['id'] ?>&csrf_token=<?= urlencode(csrf_token()) ?>" onclick="return confirm('Delete this contact?');" class="btn delete-btn">Delete</a>
            </td>
          </tr>
        <?php endwhile; ?>
      </tbody>
